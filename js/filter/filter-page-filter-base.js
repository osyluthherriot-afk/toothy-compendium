...
export class PageFilterBase {
	static defaultSourceSelFn (val) {
		return PageFilterBase.defaultSourceSelFnToothy(val);
	}

	static defaultSourceSelFnToothy (val) {
		const filterGroup = SourceUtil.getFilterGroup(val);
		return filterGroup === SourceUtil.FILTER_GROUP_PRERELEASE
			|| filterGroup === SourceUtil.FILTER_GROUP_HOMEBREW
			|| !SourceUtil.isClassicSource(val);
	}

	static defaultSourceSelFnStandardPartnered (val) {
		const filterGroup = SourceUtil.getFilterGroup(val);
		if (filterGroup === SourceUtil.FILTER_GROUP_STANDARD) return !SourceUtil.isClassicSource(val);
		if (filterGroup === SourceUtil.FILTER_GROUP_PARTNERED && (typeof BrewUtil2 === "undefined" || BrewUtil2.hasSourceJson(val))) return !SourceUtil.isClassicSource(val);
		return false;
	}
